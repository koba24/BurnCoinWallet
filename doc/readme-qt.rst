BurnCoin-qt: Qt4 GUI for BurnCoin
===============================
...
Alternatively, install Qt Creator and open the `BurnCoin-qt.pro` file.

An executable named `BurnCoin-qt` will be built.
...
A warning for people using the *static binary* version of BurnCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of BurnCoin is linked against libdb5.3.
...
installed causes BurnCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
